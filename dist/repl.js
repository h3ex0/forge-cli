...
async function runTurn(state, prompt) {
...
export async function startRepl(cfg) {
    ...
    fetchModels(state.cfg.profiles[state.cfg.activeProfile])
        ...
        for (const model of models)
            state.pricingCache.set(`${state.cfg.activeProfile}:${model.id}`, model);
    })
        .catch(() => undefined);
    while (!state.reader.isExhausted()) {
        ...
        if (state.pendingPrompt) {
            ...
            await runTurn(state, pending).catch((error) => printError(`Unexpected error: ${error instanceof Error ? error.message : String(error)}`));
            divider();
            continue;
        }
        if (cmdResult === "handled")
            continue;
        await runTurn(state, trimmed).catch((error) => printError(`Unexpected error: ${error instanceof Error ? error.message : String(error)}`));
        divider();
    }
}
