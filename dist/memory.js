 ...
 * browsable by hand.
 */
function memoryFile(workspaceRoot) {
...
export function readMemory(workspaceRoot) {
    try {
        const parsed = JSON.parse(fs.readFileSync(memoryFile(workspaceRoot), "utf-8"));
...
function writeAll(workspaceRoot, entries) {
...
export function rememberFact(workspaceRoot, text) {
...
export function forgetFact(workspaceRoot, id) {
    const entries = readMemory(workspaceRoot);
...
export function clearMemory(workspaceRoot) {
    writeAll(workspaceRoot, []);
}
/** Render memory for the system prompt, bounded so it can't dominate context. */
export function memoryPromptSection(workspaceRoot, maxChars = 4_000) {
    const entries = readMemory(workspaceRoot);
    if (!entries.length)
        return "";
    const lines = [];
