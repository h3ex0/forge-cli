...
export function MessageBlock({ message, theme, maxLines, paneWidth }) {
    ...
    return _jsxs(Box, { flexDirection: "column", marginBottom: 1, children: [_jsx(Text, { bold: true, color: color, children: role }), rows.map((line, index) => {
                const code = /^\s{4}|^```/.test(line);
                const heading = /^#{1,6}\s/.test(line);
                return _jsx(Text, { color: code ? theme.code : theme.text, bold: heading, wrap: "truncate-end", children: line || " " }, index);
