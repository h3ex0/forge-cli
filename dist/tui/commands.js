    ...
    switch (parsed.name) {
        ...
        case "context": {
            ...
            if (sub === "window") {
                ...
                if (tokens === "clear") {
                    delete profile.contextWindowTokens;
                    context.persist();
                    return { type: "notice", message: "Context-window warning cleared." };
                }
                const value = Number(tokens);
                if (!Number.isInteger(value) || value <= 0)
                    return { type: "notice", message: `Context window: ${profile.contextWindowTokens?.toLocaleString("en-US") ?? "not set"}. Usage: /context window <tokens>|clear` };
        ...
        case "branch": {
            ...
            try {
                saveSession(name, context.messages);
                return { type: "notice", message: `Conversation branched as ${name}.` };
            }
            catch (error) {
                return { type: "notice", message: error instanceof Error ? error.message : String(error) };
            }
        }
        case "export": {
            if (!arg || !/\.(md|json)$/i.test(arg))
                return { type: "notice", message: "Usage: /export <file.md|file.json>" };
        ...
        case "read": {
            if (!parsed.args[0])
                return { type: "notice", message: "Usage: /read <file> [start:end]" };
            const range = parsed.args[1]?.match(/^(\d+):(\d+)$/);
            if (parsed.args[1] && !range)
                return { type: "notice", message: "Line range must use start:end, for example 10:40." };
        ...
        case "open": {
            if (!arg)
                return { type: "notice", message: "Usage: /open <file>" };
            try {
                ...
                return { type: "notice", message: `Could not pin file: ${error instanceof Error ? error.message : String(error)}` };
            }
        }
        ...
        case "typecheck": {
            ...
            if (!script)
                return { type: "notice", message: requested ? `No npm script named ${requested} was detected.` : `No ${parsed.name} npm script was detected.` };
            return { type: "tool", name: "run_command", args: { command: process.platform === "win32" ? "npm.cmd" : "npm", args: ["run", script] } };
        }
        case "check": {
            const project = detectProject(config.permissions.workspaceRoot);
        ...
        case "usage": {
            ...
            if (parsed.args[0])
                return { type: "notice", message: "Usage: /usage [reset]" };
