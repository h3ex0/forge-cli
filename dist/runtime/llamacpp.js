...
export class LlamaCppAdapter extends OpenAICompatibleRuntimeAdapter {
    ...
    async *pullModel(id, signal) {
        ...
        if (!filename.toLowerCase().endsWith(".gguf"))
            throw new Error("llama.cpp downloads require an explicit .gguf URL.");
        ...
        if (!response.ok || !response.body)
            throw new Error(`GGUF download failed: HTTP ${response.status}`);
        const total = Number(response.headers.get("content-length")) || undefined;
        yield { status: `Downloading ${filename}`, completed: 0, total };
        try {
            await pipeline(Readable.fromWeb(response.body), fs.createWriteStream(temporary), { signal });
