...
export function estimateCost(profile, usage, pricing) {
    if (!pricing || (pricing.inputPricePerMillion == null && pricing.outputPricePerMillion == null))
        return undefined;
...
export function renderUsageStatus(config, usage, columns = 160) {
    ...
    if (subscription?.name)
        parts.push(`plan ${subscription.name}`);
    const planTokens = usage.subscriptionTokensUsed ?? total;
    if (subscription?.tokenLimit)
        parts.push(`${planTokens >= subscription.tokenLimit ? "! " : ""}plan tokens ${compactNumber(planTokens)}/${compactNumber(subscription.tokenLimit)}`);
    ...
    if (subscription?.resetAt)
        parts.push(`resets ${subscription.resetAt}`);
