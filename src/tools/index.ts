...
function strings(args: Record<string, unknown>, key: string): string[] {
  ...
  if (!Array.isArray(value) || value.some((item) => typeof item !== "string")) throw new Error(`Expected string array: ${key}`);
  return value;
}

function workspacePattern(args: Record<string, unknown>, key: string, fallback: string): string {
...
export function createTools(context: ToolContext): ToolSpec[] {
  ...
  const tools: ToolSpec[] = [
    ...
    tool(
      ...
      async (args) => {
        const file = resolveWorkspacePath(root, text(args, "path"));
        if (!fs.statSync(file).isFile()) throw new Error("Hash target must be a file.");
    ...
    tool(
      ...
      async (args) => {
        const file = resolveWorkspacePath(root, text(args, "path"));
        let value: unknown = JSON.parse(fs.readFileSync(file, "utf-8"));
        ...
        for (const segment of pointer.split("/").slice(1)) {
          ...
          if (value === null || typeof value !== "object" || !(key in value)) throw new Error(`JSON Pointer segment not found: ${key}`);
    ...
    tool(
      ...
      "read",
      async (args) => clip((await fg(workspacePattern(args, "pattern", "**/*"), { cwd: root, dot: false, onlyFiles: false, ignore: [".git/**", "node_modules/**", "dist/**"] })).join("\n")),
    ),
    tool(
      {
    ...
    tool(
      ...
      "read",
      async (args) => clip((await fg(workspacePattern(args, "pattern", "**/*"), { cwd: root, dot: false, onlyFiles: false, ignore: [".git/**", "node_modules/**"] })).join("\n") || "(no matches)"),
    ),
    tool(
      {
        name: "grep_search",
      ...
      async (args) => {
        ...
        for (const relative of files) {
          let content: string;
          try { content = fs.readFileSync(path.join(root, relative), "utf-8"); } catch { continue; }
          for (const [index, line] of content.split(/\r?\n/).entries()) {
            regex.lastIndex = 0;
            if (regex.test(line)) hits.push(`${relative}:${index + 1}: ${line.trim()}`);
    ...
    tool(
      ...
      async (args) => {
        ...
        if (!fs.statSync(source).isFile()) throw new Error("Copy source must be a file.");
    ...
    tool(
      ...
      async (args) => {
        const source = resolveWorkspacePath(root, text(args, "source"));
        const destination = resolveWorkspacePath(root, text(args, "destination"), { allowMissing: true });
        if (fs.existsSync(destination)) throw new Error(`Destination already exists: ${path.relative(root, destination)}`);
        fs.mkdirSync(path.dirname(destination), { recursive: true });
    ...
    tool(
      ...
      async (args) => {
        const file = resolveWorkspacePath(root, text(args, "path"));
        if (!fs.statSync(file).isFile()) throw new Error("delete_file only removes files; use run_command for directories.");
        fs.unlinkSync(file);
    ...
    tool(
      ...
      async (args) => {
        ...
        if (!Array.isArray(raw)) throw new Error("Expected array argument: items");
    ...
    tool(
      ...
      "read",
      async () => renderTodos(todoStore.get(root) ?? []),
    ),
    tool(
      {
        name: "memory_write",
...
export function findTool(name: string, context: ToolContext = { workspaceRoot: process.cwd() }): ToolSpec | undefined {
