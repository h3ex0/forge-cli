...
export type UsageLedger = z.infer<typeof ledgerSchema>;

export const USAGE_PATH = path.join(os.homedir(), ".forge", "usage.json");

function saveUsageLedger(ledger: UsageLedger, file: string): void {
...
export function loadUsageLedger(file = USAGE_PATH): UsageLedger {
  try { return ledgerSchema.parse(JSON.parse(fs.readFileSync(file, "utf-8"))); }
  catch { return { schemaVersion: 1, profiles: {} }; }
}

/** Atomically add provider-reported token usage to one profile's local ledger. */
export function recordUsage(profile: string, promptTokens: number, completionTokens: number, file = USAGE_PATH): UsageLedger {
...
export function clearProfileUsage(profile: string, file = USAGE_PATH): void {
  const ledger = loadUsageLedger(file);
