...
export interface Skill {
  name: string;
  description: string;
  /** Body of the skill file, with any frontmatter stripped. */
  body: string;
  source: string;
  /** "workspace" wins over "user" when both define the same name. */
 ...
 */
function skillDirs(workspaceRoot: string): Array<{ dir: string; scope: Skill["scope"] }> {
  return [
...
function parseSkill(file: string, scope: Skill["scope"]): Skill | undefined {
  let raw: string;
  try { raw = fs.readFileSync(file, "utf-8"); } catch { return undefined; }
...
export function listSkills(workspaceRoot: string): Skill[] {
  ...
  for (const { dir, scope } of skillDirs(workspaceRoot)) {
    ...
    for (const entry of entries.sort()) {
      ...
      if (!found.has(skill.name.toLowerCase())) found.set(skill.name.toLowerCase(), skill);
    }
  }
  return [...found.values()].sort((a, b) => a.name.localeCompare(b.name));
}

export function findSkill(workspaceRoot: string, name: string): Skill | undefined {
...
export function skillsPromptSection(workspaceRoot: string): string {
  const skills = listSkills(workspaceRoot);
